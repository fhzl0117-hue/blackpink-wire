...
An independent, unofficial English-language fan hub for BLACKPINK/BLINK — news translation, album reviews, official-only video curation, a timezone-aware release schedule, and a fan quiz, built to run on Google AdSense.

**Not affiliated with, endorsed by, or sponsored by YG Entertainment, THEBLACKLABEL, ADOR, or the members of BLACKPINK.**

## What's in this repo

This is a single static page — `index.html` — with everything (HTML, CSS, JS) inlined. No build step, no dependencies, no backend. It's meant to be served as-is by GitHub Pages. This repo follows the same template as [Bangtan Wire](https://github.com/fhzl0117-hue/bangtan-wire) — part of the "Wire" series, one dedicated site per artist.
...
| 04 · Signal | `#signal` | Release/comeback schedule with a live countdown, auto-converted to the visitor's local timezone |
...
Everything is plain HTML — open `index.html` in any editor and look for the section with the matching `id` (e.g. `<section ... id="news">`) to update copy. There's no CMS yet; each dispatch, review, or release date is a hand-edited block. See the comments inside the `<script>` tag at the bottom for how the quiz and countdown timers work if you need to change their logic.

**Before adding new release dates or news items**, verify the underlying facts against a real source and keep the "Read the original source" link pointing at it — that link is what keeps this page compliant with content policies (Google AdSense does not allow re-publishing copyrighted material, and this page's whole design is built around linking out and summarizing instead of reposting).
